...
def build_parser() -> argparse.ArgumentParser:
    ...
    handoff = sub.add_parser(
        ...
        help=(
            ...
            "repo artifacts and write an advisory handoff descriptor "
            "to `.agent-loop/prompt-handoff.json` capturing which "
            "prompt was dispatched, when, in which mode, plus the "
            "active phase / sub-phase / cycle / approval-mode "
            "context. Writes only the descriptor and a `prompt "
            "handoff:` audit-log line; never modifies the canonical "
            "prompt artifacts, loop-state, or any other protected "
            "artifact. Refuses fail-closed on missing / malformed "
            "loop-state, unsupported `contract_version`, out-of-"
            "vocabulary `--mode`, missing or empty prompt artifact, "
            "and output-boundary violations (path outside "
            "`.agent-loop/`, protected path, memory subtree, "
            "directory)."
        ),
    )
