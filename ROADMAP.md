...
- Phase 10AF - Desktop Codex Conversation And Intervention Contract: define how the desktop app may expose a bounded in-app conversation surface to Codex so the operator can ask for reviews, fixes, roadmap changes, or targeted repo changes without bypassing canonical artifacts, ownership rules, or review evidence
- Phase 10AG - Desktop Codex Conversation Surface Initial Slice: implement the first desktop-side Codex interaction panel so the operator can type a request to Codex, inspect Codex responses, and route approved Codex-owned actions through the shipped adapter/artifact model instead of separate chat windows
- Phase 10AH - Orchestration Graph And Phase-State Visualization Contract: define how the desktop app should present the live orchestration state as a node/edge graph or icon-based flow view, including phase/sub-phase, current loop state, review/fix branch, blocked/halted states, and artifact-backed progress without inventing UI-only truth
- Phase 10AI - Orchestration Graph And Performance View Initial Slice: implement the first visual orchestration map in the desktop app, inspired by node-based workflow tools, so the operator can see where the agent currently is in the loop, what step just completed, what is waiting next, and where the run is blocked or halted
