...
Phase 5B (Review Mode Initial Slice, shipped) makes `review` the explicit default approval mode and adds the first machine-readable Claude completion handoff artifact. Only the `review` path is implemented; `strict` and `autonomous` runtime branching remain deferred to later 5x slices. New / freshly activated Phase 5+ runtime state in `.agent-loop/loop-state.json` carries `approval_mode = "review"` and `awaiting_human_for = null` by default (a pre-existing non-review `approval_mode` is preserved across activation; `awaiting_human_for` is always reset by activation). During a normal cycle the orchestrator backfills the same defaults on any loaded state that pre-dates Phase 5B, and on the implemented `review` path it sets `awaiting_human_for = "phase_complete_awaiting_human_approval"` on `APPROVED_FOR_HUMAN_REVIEW` and leaves it `null` everywhere else. After Claude returns a fresh `claude-summary.md` for either an implementation or fix cycle, the orchestrator writes `.agent-loop/claude-done.json` (`signal_version`, `phase`, `sub_phase`, `task`, `cycle_count`, `mode` of `"implementation"` or `"fix"`, `source_prompt_path`, and `status = "ready_for_codex_review"`) as a routing/timing signal only; it never substitutes for `claude-summary.md`, the git diff, or validation evidence in review. Each new prompt or fix-prompt issuance clears any prior `claude-done.json` so a stale completion signal cannot drive the wrong review cycle. The rest of the baseline `review`-mode loop is preserved unchanged: Codex still authors prompts, evidence still runs before review, Codex review still drives verdicts, phase progression still requires human approval, and there is no commit/push automation.
