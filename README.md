...
Phase 5D is the active approval-mode slice. Its scope is the first narrow `autonomous` runtime continuation path. When `approval_mode = "autonomous"`, the target behavior for this slice is: bypass each of the four Phase 5C strict-mode human gates (`pre_claude_prompt`, `pre_codex_review` for the implementation cycle, `pre_fix_prompt`, `pre_codex_review` for the fix cycle) and continue directly through the existing implementation step, the existing Claude-completion-to-Codex-review handoff, and bounded `NEEDS_FIXES` fix cycles within the existing `cycle_count` / `max_cycles` threshold. Each gate bypass should write a single `autonomous mode: bypassing <gate> gate at <where>` note to `.agent-loop/orchestrator.log` so the mode is auditable from repo artifacts and not hidden in transient control flow. Every existing hard stop must remain preserved: `FAILED_REQUIRES_HUMAN` still halts immediately, malformed evidence or summaries still fail closed, the `cycle_count >= max_cycles` threshold on `NEEDS_FIXES` still halts, and the `APPROVED_FOR_HUMAN_REVIEW` terminal must still persist `status = phase_complete_awaiting_human_approval` and `awaiting_human_for = phase_complete_awaiting_human_approval` because the Phase 5A contract requires human approval before phase progression. The shipped `review` (Phase 5B) and `strict` (Phase 5C) behavior must remain unchanged.
