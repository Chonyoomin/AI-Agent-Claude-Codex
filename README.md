...
Phase 4F makes that seam selectable. `make_planner_adapter()` now chooses the adapter from the `AGENT_LOOP_PLANNER_CMD` environment variable: when it holds a non-blank command the alternate `SubprocessPlannerAdapter` is used (it runs that command through the platform shell with the repository root as cwd and passes its exit code through, using the same `0` = proposal written / `2` = refusal convention as the in-process planner, and a missing shell maps to exit `127`); when the variable is unset, empty, or whitespace-only, the factory falls back to the default `LocalPlannerAdapter` so behavior is unchanged out of the box. Exactly one alternate adapter exists. Selection is purely a dispatch choice: the configured command is expected to honor the unchanged planner write boundary, the seam itself never writes any file, never widens the planner or activation write surface, and never activates a proposal. Activation stays the separate, human-controlled `activate` step - planning (proposal generation and refresh) and activation remain distinct regardless of which adapter is selected.
...
- `tests/test_planner_adapter_selection.py` covering the Phase 4F adapter selection: no / blank / whitespace-only `AGENT_LOOP_PLANNER_CMD` falls back to `LocalPlannerAdapter`, a non-blank command selects `SubprocessPlannerAdapter`, the subprocess adapter passes success / refusal exit codes through and maps a missing shell to `127`, and selecting the alternate adapter for a `plan` run performs no activation-owned writes and does not touch `loop-state.json`

Deferred to later phases generally: approval modes (Phase 5), optional context and tool layer (Phase 6), editor integration (Phase 7), documentation and project polish (Phase 8). Phase 4D was the first slice that intentionally extended the Phase 3 orchestrator surface with the contained post-approval planner hook; Phase 4E routed that hook and the `plan` path through a single planner-adapter boundary; Phase 4F makes that boundary selectable between the default local adapter and one alternate subprocess adapter, while keeping the default behavior, the planner write boundary, and the separate human-controlled approval/activation steps unchanged.
