...
The instruction contract, prompt formats, review formats, task and phase ownership rules, and safety model are in place. Phase 1 (Manual File-Based Loop), Phase 2A (Evidence Collection Contract), Phase 2B (`scripts/run_checks.sh`), Phase 3 (Scripted Orchestrator MVP, delivered as sub-phases 3A through 3E), Phase 4A (Planning Contract), Phase 4B (Planner Initial Slice; the `plan` subcommand), Phase 4C (Planner Activation Writes; the `activate` subcommand), Phase 4D (Planner-Orchestrator Integration; the contained post-approval planner refresh), Phase 4E (Planner Adapter Seam; routing both planner call sites through `make_planner_adapter()`), Phase 4F (Planner Adapter Selection; adding one alternate adapter path while preserving the planner write boundary), Phase 5A (Approval Modes Contract), and Phase 5B (Review Mode Initial Slice) are complete; the historical sub-phase record is preserved in `.agent-loop/phase-plan.md`. The project is currently in **Phase 5C - Strict Mode Pauses**: the second implementation slice for approval modes, focusing on strict human checkpoints before implementation prompt dispatch, before fix-prompt dispatch, and after Claude completion but before Codex review, while preserving the shipped `review`-mode baseline and deferring `autonomous` runtime branching.
