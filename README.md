...
<<<<<<< Updated upstream
The instruction contract, prompt formats, review formats, task and phase ownership rules, and safety model are in place. Phase 1 (Manual File-Based Loop), Phase 2A (Evidence Collection Contract), Phase 2B (`scripts/run_checks.sh`), Phase 3 (Scripted Orchestrator MVP, delivered as sub-phases 3A through 3E), Phase 4A (Planning Contract), Phase 4B (Planner Initial Slice; the `plan` subcommand), Phase 4C (Planner Activation Writes; the `activate` subcommand), and Phase 4D (Planner-Orchestrator Integration; the contained post-approval planner refresh) are all complete; the historical sub-phase record is preserved in `.agent-loop/phase-plan.md`. The project is currently in **Phase 4E - Planner Adapter Seam**: routing planner execution through a dedicated adapter boundary so the `plan` path and the post-approval refresh no longer hard-wire a direct in-process call, while the default adapter preserves today's behavior. Selecting an alternate (for example out-of-process) planner adapter remains deferred.
=======
The instruction contract, prompt formats, review formats, task and phase ownership rules, and safety model are in place. Phase 1 (Manual File-Based Loop), Phase 2A (Evidence Collection Contract), Phase 2B (`scripts/run_checks.sh`), Phase 3 (Scripted Orchestrator MVP, delivered as sub-phases 3A through 3E), Phase 4A (Planning Contract), Phase 4B (Planner Initial Slice; the `plan` subcommand), Phase 4C (Planner Activation Writes; the `activate` subcommand), Phase 4D (Planner-Orchestrator Integration; the contained post-approval planner refresh), Phase 4E (Planner Adapter Seam; routing both planner call sites through `make_planner_adapter()`), and Phase 4F (Planner Adapter Selection; adding one alternate adapter path while preserving the planner write boundary) are all complete; the historical sub-phase record is preserved in `.agent-loop/phase-plan.md`. The project is currently in **Phase 5A - Approval Modes Contract**: a Codex-owned contract-definition slice that specifies `strict`, `review`, and `autonomous` mode behavior, including when humans must approve, when Codex may issue implementation or fix prompts, when Claude completion hands the loop back for review, and how a future `.agent-loop/claude-done.json` signal participates in that routing. Approval modes are not implemented runtime behavior yet.
>>>>>>> Stashed changes
...
Deferred to later 4x sub-phases: an alternate (for example out-of-process) planner adapter behind the Phase 4E seam. Deferred to later phases generally: approval modes (Phase 5), optional context and tool layer (Phase 6), editor integration (Phase 7), documentation and project polish (Phase 8). Phase 4D was the first slice that intentionally extended the Phase 3 orchestrator surface with the contained post-approval planner hook; Phase 4E routes that hook and the `plan` path through a single planner-adapter boundary while keeping the default behavior, the planner write boundary, and the separate human-controlled approval/activation steps unchanged.
