...
The instruction contract, prompt formats, review formats, task and phase ownership rules, and safety model are in place. Phase 1 (Manual File-Based Loop), Phase 2A (Evidence Collection Contract), Phase 2B (`scripts/run_checks.sh`), Phase 3 (Scripted Orchestrator MVP, delivered as sub-phases 3A through 3E), Phase 4A (Planning Contract), and Phase 4B (Planner Initial Slice; the `plan` subcommand) are all complete; the historical sub-phase record is preserved in `.agent-loop/phase-plan.md`. The project is currently in **Phase 4C - Planner Activation Writes**: implementing the activation step the Phase 4A contract authorizes after explicit human approval. Planner-orchestrator auto-integration (Phase 4D) and the optional planner adapter remain deferred.
...
4. Activation is NOT folded into `plan`, and the planner is NOT auto-invoked by `run_normal_cycle` or `_run_fix_cycle`; planner-orchestrator integration is deferred to Phase 4D.

Focused tests live in:
- `tests/test_planner.py` (29 tests) covering the major Phase 4A refusal paths, the bounded-scope self-validation, the additional input-loading completeness, and the full proposal-generation success path; run with `python tests/test_planner.py`.
- `tests/test_planner_activation.py` (26 tests) covering the activation success path, every approval-signal refusal (no `## Approval`, empty body, wrong case, extra words, leading marker, label mismatch, token outside `## Approval`), every structural refusal (missing/malformed `loop-state.json`, missing proposal, empty required-proposal section), the loop-state reset semantics, the TASK.md preservation rules, the phase-plan.md append-only invariant, the approval-source log entry, and the activator's write-boundary discipline; run with `python tests/test_planner_activation.py`.

Deferred to later 4x sub-phases: planner-orchestrator auto-integration (Phase 4D), optional planner adapter (Phase 4E or later). Deferred to later phases generally: approval modes (Phase 5), optional context and tool layer (Phase 6), editor integration (Phase 7), documentation and project polish (Phase 8). The Phase 3 orchestrator surface (`scripts/agent_loop.py` normal cycle / fix cycle / adapters, `scripts/run_checks.sh`) is unchanged by Phase 4C beyond the addition of activator functions and the `activate` subcommand.
