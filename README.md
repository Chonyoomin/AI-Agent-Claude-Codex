...
The instruction contract, prompt formats, review formats, task and phase ownership rules, and safety model are in place. Phase 1 (Manual File-Based Loop), Phase 2A (Evidence Collection Contract), Phase 2B (`scripts/run_checks.sh`), Phase 3 (Scripted Orchestrator MVP, delivered as sub-phases 3A through 3E), Phase 4A (Planning Contract), Phase 4B (Planner Initial Slice; the `plan` subcommand), Phase 4C (Planner Activation Writes; the `activate` subcommand), Phase 4D (Planner-Orchestrator Integration; the contained post-approval planner refresh), and Phase 4E (Planner Adapter Seam; routing both planner call sites through `make_planner_adapter()`) are all complete; the historical sub-phase record is preserved in `.agent-loop/phase-plan.md`. The project is currently in **Phase 4F - Planner Adapter Selection**: making the seam selectable so planner execution uses either the default in-process local adapter or one alternate, out-of-process subprocess adapter, with the local adapter remaining the fallback when no (or invalid) alternate is configured.
...
Phase 4E adds a planner-adapter seam. Both the `plan` path (step 1) and the post-approval planner refresh (step 4) dispatch planner execution through `make_planner_adapter().run(...)` instead of calling the planner in-process directly. The default adapter (`LocalPlannerAdapter`) runs the same in-process planner, so behavior, refusal handling, the exception-containment guarantees on the post-approval path, and the planner's write boundary (`.agent-loop/proposed-phase.md` and `.agent-loop/planner.log` only) are all unchanged. The adapter is purely a dispatch boundary: it never widens the planner write boundary and never activates a proposal.

Phase 4F makes that seam selectable. `make_planner_adapter()` chooses the adapter from the `AGENT_LOOP_PLANNER_CMD` environment variable. When it holds a statically-valid command the alternate `SubprocessPlannerAdapter` is used; otherwise the factory falls back to the default `LocalPlannerAdapter` so behavior is unchanged out of the box. Exactly one alternate adapter exists. Activation stays the separate, human-controlled `activate` step - planning (proposal generation and refresh) and activation remain distinct regardless of which adapter is selected.

The alternate adapter ENFORCES the planner write boundary rather than trusting the configured command. It snapshots the in-scope repository, runs the command through the platform shell with the repository root as cwd, and then checks what changed. If the command stayed within the allowed planner set (`.agent-loop/proposed-phase.md` and `.agent-loop/planner.log` only) the adapter keeps those writes and passes the command's exit code through (same `0` = proposal written / `2` = refusal convention as the in-process planner; a missing shell maps to exit `127`). If the command created, modified, or deleted ANY other file - in particular any activation-owned file (`TASK.md`, `.agent-loop/current-task.md`, `.agent-loop/current-phase.md`, `.agent-loop/phase-plan.md`, `.agent-loop/loop-state.json`) - the adapter rolls the repository back to the snapshot (reverting even the allowed writes so a violating run leaves no trace), logs a `note:` line to `.agent-loop/planner.log`, and fails closed with exit `2`. So an alternate-adapter run can never leave behind a write outside the planner boundary, and never auto-activates. (Volatile / VCS directories such as `.git` and `__pycache__` are excluded from the snapshot scope as regenerable noise.)

"Statically-valid command" means a non-blank string that parses into at least one token. Invalid configuration - unset, empty, whitespace-only, or unparseable (for example an unbalanced quote) - falls back to the local adapter. A command that is well-formed but fails when executed is NOT a fallback case: it is selected and surfaced through its exit code, or, if it breaches the write boundary, through the fail-closed rollback above. True "will this command work" fallback is intentionally not attempted, because it cannot be decided without running the command (and running it only to fall back could leave side effects).
...
- `tests/test_planner_adapter_selection.py` covering the Phase 4F adapter selection and write-boundary enforcement: invalid configuration (unset / blank / whitespace-only / unparseable `AGENT_LOOP_PLANNER_CMD`) falls back to `LocalPlannerAdapter` while a parseable-but-broken command is still selected; the subprocess adapter passes success / refusal exit codes through and maps a missing shell to `127`; an out-of-bound write (a new file, or any activation-owned file such as `TASK.md` or `loop-state.json`) fails closed with exit `2` and is rolled back so the repo matches the pre-run snapshot and no partial proposal remains; in-bound writes are kept; the snapshot / detect / restore helpers are exercised directly; and the post-approval path with selection active is not regressed (a clean run logs exit 0, a boundary-violating run rolls back and logs exit 2 without raising)

Deferred to later phases generally: approval modes (Phase 5), optional context and tool layer (Phase 6), editor integration (Phase 7), documentation and project polish (Phase 8). Phase 4D was the first slice that intentionally extended the Phase 3 orchestrator surface with the contained post-approval planner hook; Phase 4E routed that hook and the `plan` path through a single planner-adapter boundary; Phase 4F makes that boundary selectable between the default local adapter and one alternate subprocess adapter that enforces the planner write boundary (failing closed with a rollback on any out-of-bound write), while keeping the default behavior, the planner write boundary, and the separate human-controlled approval/activation steps unchanged.
