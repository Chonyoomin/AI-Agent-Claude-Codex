...
The instruction contract, prompt formats, review formats, task and phase ownership rules, and safety model are in place. Phase 1 (Manual File-Based Loop), Phase 2A (Evidence Collection Contract), and Phase 2B (`scripts/run_checks.sh`) all closed with `APPROVED_FOR_HUMAN_REVIEW`. The project is currently in **Phase 3A - Orchestrator Contract**: planning and documentation for the Scripted Orchestrator MVP. The contract lives in `.agent-loop/phase-plan.md` under `## Phase 3A - Orchestrator Contract` and specifies the inputs, allowed writes, cycle order of operations, verdict handling, loop-state updates, and stop conditions that `scripts/agent_loop.py` must satisfy. The orchestrator itself (`scripts/agent_loop.py`, Phase 3B) is not yet implemented.
