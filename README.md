...
The instruction contract, prompt formats, review formats, task and phase ownership rules, and safety model are in place. Phase 1 (Manual File-Based Loop), Phase 2A (Evidence Collection Contract), Phase 2B (`scripts/run_checks.sh`), Phase 3A (Orchestrator Contract), Phase 3B (initial Orchestrator MVP slice), and Phase 3C (automated fix-cycle handling) are all complete and approved by the human. The project is currently in **Phase 3D - Subprocess-Driven Claude/Codex Adapters**: replacing the manual-handoff Claude and Codex adapter stubs in `scripts/agent_loop.py` with real subprocess-driven adapters that invoke the user's configured Claude / Codex CLI command, while preserving the verdict loop, the fix-cycle, the fail-closed validators, the threshold-policy halt, the `halted_human_stop` persistence, and the `codex_version` null-note behavior unchanged. The manual-handoff adapters remain available as the fallback when the user has not configured a subprocess command, so the documented manual workflow continues to work.
